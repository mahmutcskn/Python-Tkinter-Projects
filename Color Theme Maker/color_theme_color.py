...
def get_red(slider_value):
    """Turn current slider value for red ito a hex value and update color
    The scale value is passed automatically when the scale is moved calling the get_red function"""

    global red_value

    #Turn the slider calue into an int and hex value. Strip leading chars so only two remain
...
def get_green(slider_value):
    """Turn current slider value for green ito a hex value and update color
...
def get_blue(slider_value):
    """Turn current slider value for blue ito a hex value and update color
...
def update_color():
    """Update the current color bx based on the slider values. Display tuple and hex values of the current color"""
...
def store_color():
    """Store the current color tuple calue and display color"""
    global stored_colors

    #Get the current alue of each slider and append 0's to keep formatting
    ...
    #Put new widgets oon the screen
    ...
    new_color_black_box.grid(row=stored_color.get(),column=4,padx=20,ipadx=5,ipady=5)
    ...
    #Move the radio button stored colors_ to the next value if avialable
...
def save_colors():
    ...
    with open(file_name, "w") as f:
        f.write("Color Theme Master Output\n")
...
#Make a buttons for each complimentary color
...
#Make the color box and color labels
...
#Make radio buttons to select stored colors and populate each row with placeholder values
for i in range(6):
