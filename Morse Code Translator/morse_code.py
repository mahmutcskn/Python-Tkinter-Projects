...
def get_morse():
    ...
    #Get the input text of symbols not in our dict keys
    ...
    #Break up into individual words based on space " " and put into list
...
def get_english():
    """Convert a morse codee message to english"""
    ...
    for word in word_list:
        letters = word.split(" ")
        #For each word, get the english representation and append it to the english
        for letter in letters:
...
def play():
    ...
    #Determine where the morse code it
    if language.get() == 1:
...
#Make our morse code dictionaries
...
output_text = tkinter.Text(output_frame,height=8,width=30,bg=button_color)
