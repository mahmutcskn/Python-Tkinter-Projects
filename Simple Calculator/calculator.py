...
def operate(operator):
    ...
    #Disable all operator buttons until equaş or clear is pressed
    ...
    #Return dec,mal button to normal state
...
def square():
    ...
    #Remove the current calue in the display and update it with the answer
    display.delete(0,END)
    display.insert(0,value)
...
multiple_button = tkinter.Button(button_frame,text="*",font=button_font, bg=light_green,command=lambda:operate('multiply'))
substract_button = tkinter.Button(button_frame,text="-",font=button_font, bg=light_green,command=lambda:operate('subtract'))
...
nagate_button = tkinter.Button(button_frame,text="+/-",font=button_font, bg='black',fg='white',command=negate)
...
multiple_button.grid(row=2,column=3,pady=1,sticky="WE",ipadx=20)
#Fourth row
...
#sixth Row
nagate_button.grid(row=5,column=0,pady=1,sticky="WE")
